...
local function setup_native(server_id)
...
if mlsp.setup_handlers then
    mlsp.setup_handlers({
        function(server_id) setup_native(server_id) end
    })
else
    for _, server_id in ipairs(mlsp.get_installed_servers()) do
        setup_native(server_id)
    end
end
